...
\newglossaryentry{ARPA-gls}{
   ...
   description = {
      ...
      Also known as \SC{DARPA}, the `D' standing for Defense.
   }
}
...
\newglossaryentry{authentication server}{
   ...
   description = {The authentication server verifies the provided username and password and assigns the correct permissions to the user.}
}

\newglossaryentry{authenticator}{
   name = {authenticator},
   description = {The network device to which a client wants to connect -- usually a switch or wireless access point. The authenticator asks the client for credentials and forwards them to the authentication server.}
}
...
\newglossaryentry{channel access method}{
   name = {channel access method},
   description = {
      A channel access method or \emph{multiple access method} allows more than two terminals connected to the same transmission medium to transmit over it and to share its capacity.
...
\newglossaryentry{collision domain}{
   name = {collision domain},
   description = {A shared network segment in which collisions can occur.}
}
...
\newglossaryentry{convergence time}{
   name = {convergence time},
   description = {The time required for a set of routers to have the same topological information about the network in which they operate.}
}
...
\newglossaryentry{datagram}{
   ...
   description = {
      ...
      A datagram contains all of the inforamtion needed to deliver it to its destination, analogous to a letter in the \SC{US} postal system.
      Datagram networks are connectionless.
   }
}

\newglossaryentry{DDoS-gls}{
   name = {\acf{DDoS}},
   sort = {distribute denial-of-service},
   description = {
      A \ac{DDoS} attack is a cyber-attack in which the perpetrator seeks to make a machine or network resource unavailable to its intended users by temporarily or indefinitely disrupting services of a host connected to a network.
...
\newglossaryentry{leased line}{
   ...
   description = {
      ...
      Unlike traditional telephone lines in the \ac{PSTN} leased lines are generally not switched circuits, and therefore do not have an associated telephone number.
      Each side of the line is permanently connected, always active and dedicated to the other.
...
\newglossaryentry{modem}{
   name = {modem},
   description = {
      A \emph{modulator--demodulator} or \emph{modem} is a computer hardware device that converts data from a digital format into a format suitable for an analog transmission medium such as telephone or radio.
...
\newglossaryentry{port mirroring}{
   name = {port mirroring},
   description = {
      Port mirroring is used on a network switch to send a copy of network packets seen on one switch port (or an entire \acs{VLAN}) to a network monitoring connection on another switch port.
      This is commonly used for network appliances that require monitoring of network traffic such as an \acl{IDS}.
      Port mirroring on a Cisco Systems switch is generally referred to as \acf{SPAN} or \acf{RSPAN}.
      Other vendors have different names for it.
   }
}
...
\newglossaryentry{supplicant}{
   name = {supplicant},
   description = {an entity at one end of a point-to-point \acs{LAN} segment that seeks to be authenticated by an authenticator attached to the other end of that link.}
}
...
\newglossaryentry{token}{
   name = {token},
   description = {
      A token is a special type of frame used in logical ring topologies such as Token Ring.
      When no station is sending a frame, this special token frame circles the loop.
      The token is repeated from station to station until arriving at a station that needs to send data. 
   }
}
