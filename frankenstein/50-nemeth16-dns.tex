...
\Cref{fig:simple-name-lookup} illustrates a typical scenario.
...
opposed to other name lookup methods such as a static \filename{/etc/hosts} file.


%\subsection{\filename{resolv.conf}: client resolver configuration}
\subsection{resolv.conf: client resolver configuration}

Each host on the network should be a DNS client.
You configure the client-side resolver in the file \filename{/etc/resolv.conf}.
...
The name servers listed in \filename{resolv.conf} must be configured to allow
...
Both FreeBSD and Linux use a switch file, \filename{/etc/nsswitch.conf}, to specify how hostname-to-IP-address mappings should be performed and
...
the next entry (in this case, the \filename{/etc/hosts} file).
If no name server is running (as might be the case during boot), the lookup process does consult the \filename{hosts} file.

Our example distributions all provide the following default \filename{nsswitch.conf} entry:
\begin{config}
hosts: files dns
\end{config}

This configuration gives precedence to the \filename{/etc/hosts} file, which is always checked.
DNS is consulted only for names that are unresolvable through \filename{/etc/hosts}.
...
information as possible in DNS but always preserve the ability to fall back to the static \filename{hosts} file during the boot process if necessary.

If name service is provided for you by an outside organization,
you might be done with DNS configuration after setting up \filename{resolv.conf} and \filename{nsswitch.conf}.
...
reverse branch.
\Cref{fig:dns-zone-tree} shows the general layout of the naming tree.
...
To allow the same DNS system to manage both names (which have the most significant information on the right), and IP addresses (which have the
...
country code domains (ccTLDs)\index{ccTLD} and generic top-level domains (gTLDs)\index{gTLD}.
ICANN,\index{ICANN} the Internet Corporation for Assigned Names and Numbers,
...
To register for a ccTLD name, check the IANA\index{IANA}
...
The procedure for creating a subdomain is similar to that for creating a second-level domain, except that the central authority is now local (or
...
Name servers deal with ``zones,'' where a zone is essentially a domain minus its subdomains.\index{zone (DNS)}
...
\Cref{tab:ns-taxonomy} lists some of the adjectives used to describe name servers.
...
\begin{table}
   ...
   % \footnote{Strictly speaking, ``nonauthoritative'' is an attribute of a DNS query response, not a server.}
...
The \emph{master} server\index{name server!master}
...
A \emph{slave} server\index{name server!slave}
...
A \emph{caching-only}\index{name server!caching-only}
