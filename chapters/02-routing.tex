...
In \cref{fig:routing-ospf-lsa-1} \hostname{blue} will send out a type-1 LSA to its neighbours in which it provides them the following information:
\begin{itemize}
\item
   its \emph{router ID}.
   Every router must have a unique identifier.
   This is a 32-bit number, formatted as a dotted decimal number and usually taken as a loopback \abbr{IP} address so that an administrator can ping the router ID when troubleshooting network issues.
\item 
   connected interfaces.
   As \hostname{blue} connects to three different networks, it provides this information in its updates.
\end{itemize}

\begin{figure}
   \centering
   \fbox{\parbox{.7\textwidth}{
      Router \hostname{blue} connects to three other routers.
      Router \hostname{red} will forward this information to its neighbours.
   }}
   \caption{Router \hostname{blue} sends out a type-1 \abbr{LSA} telling all other routers about its connectivity}
