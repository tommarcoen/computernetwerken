...
Wide-area networks are often established with \emph{leased telecommunication circuits}.\index{telecommunication circuit}
...
% TODO: add an `extra information' paragraph about a NAS and the difference between NAS and SAN.
   \iacs{NAS}
\Gls{NAS} is file-level storage such as Windows file shares using the \acs{SMB} protocol and is not the same as a \acs{SAN}.
...
\mode<article>{
The most simple method of connecting two devices together in a network is to directly connec them together using a cable cable of transfering data.
}
