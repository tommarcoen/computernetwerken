...
A \acs{DHCP} server can be configured to always provide the same client (\acs{MAC} address) with the same \acs{IP} address.
...
To prevent fragmentation, path \acs{MTU} discovery can be used; see \vpageref{par:pmtud}.
...
\extrap{administrative distance}
