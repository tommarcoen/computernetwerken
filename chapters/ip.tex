...
This results in the logical division of an IP address into two fields: the \emph{network number} or \emph{routing prefix} and the \emph{rest field} or \emph{host identifier}.
...
As it does not really signify some time spent in transit, with the development of IPv6, they changed the name of this field to \emph{hop limit} as that is what it really does: limit the number of hops (routers) that a packet can travel.
}
...
Finally, you will also encounter fragmentation on IPsec virtual networks where the maximum transmission unit is slightly smaller than the normal 1500~bytes.
}
...
For example, on Cisco routers the administrative distance or credibility of RIP is 110 while the administrative distance of OSPF is 90.
As lower values are preferred, when a router running both RIP and OSPF learn about the same route from both protocols, it will prefer the route learnt from OSPF over the route learnt from RIP.
}
...
This ensures that the router originally advertising the prefix can never use its neighbors to reach the given network.
}
...
Logically you can use both both methods. Either you use neither method, or you use one or the other.
}

\Paragraph{hold-down timer}
\mode<article>{
Holddown works by having each router start a timer when they first receive information about a network that is unreachable.
