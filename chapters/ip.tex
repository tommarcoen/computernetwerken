...
\footnote{Currently all bytes are also eight bits so the words octet and byte are interchangeable.
    ...
    For example, the \abbr{PDP-7}, a predecessor of the \abbr{PDP-11}, was an 18-bit machine.
    }
}
...
\mode<article>{
Every (sub)network has its own broadcast address as the last \acs{IP} address in the range.
...
For example, the \acs{IP} address 198.51.100.37 falls into the class~C range as $192\le198\le223$ so the first three octets determine the network and the fourth octet identifies the host within the given network.
}
\newsavebox{\classful}
\savebox{\classful}{
   \sffamily
...
These stages (see \vref{fig:dora}) are often abbreviated as \acs{DORA} for discovery, offer, request, and acknowledgement.
...
In a typical configuration, a local network uses one of the designated private \acs{IP} address subnets (\acs{RFC}~1918).
...
\acs{RFC}~2663 uses the term \gls{NAPT} for this type of \acs{NAT}.
...
As it is an 8-bit field, the maximum value is~255 but a recommended initial value is~64.\footnote{See \acs{RFC}~1700, p.~64.}
