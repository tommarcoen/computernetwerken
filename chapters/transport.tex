...
\paragraph{header}
The \acs{TCP} header is quite a bit more complex than the \acs{UDP} header.
Just like the \acs{UDP} header in \vref{fig:transport-udp-header} it contains source and destination port numbers but in order to guarantee \emph{reliability} it needs to contain a lot more information.
See \cref{fig:transport-tcp-header} for the header format.



\begin{figure}
\begin{sidecaption}{The \acs{TCP} header}[fig:transport-tcp-header]
\centering
\begin{bytefield}{32}
\bitheader{0,15,16,31} \\
\bitbox{16}{source port}
...
\extras{Other transport protocols}
