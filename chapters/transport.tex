...
\section{Exercises}
%\begin{enumerate}
\begin{exercise}[subtitle={Three-way handshake}]
Use Wireshark to make a capture of a \acs{TCP} session and observe the three-way handshake.
\end{exercise}
\begin{exercise}[subtitle={\acs{TCP} options}]
%\item
Capture a few three-way handshakes for different protocols (e.g.~a \acs{HTTP} session, an \acs{SSH} session, and an \acs{FTP} session) and compare the different \acs{TCP} options used.
\end{exercise}
%\end{enumerate}
