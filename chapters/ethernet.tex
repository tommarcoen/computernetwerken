...
Sometimes%
   \footnote{As far as I know only Cisco uses this different format.}
the address is displayed as three groups of four hexadecimal digits, separated by dots, e.g. 0200.33aa.bbcc.%
...
The right-most bit, when set to one, indicates that the \acs{MAC} address is used for multicast or broadcast traffic.
...
See also \emph{fragmentation} on p.~\pageref{par:ip-fragmentation}.
...
The header contains the source and destination \acs{MAC} addresses, a field designated for either the \emph{ethertype} or the length of the data payload,%
   \footnote{%
...
\paragraph{Ethernet trailer}
   ...
   \iacs{FCS}
   \iacs{CRC}
The frame ends with a \acf{FCS}, which is a 32-bit \acf{CRC} used to detect and correct any in-transit corruption of data.
...
These frames can be up to 9000 bytes in size.%
   \footnote{For example the Juniper \SC{QFX} series of switches support frames up to {9216}~bytes.}
...
\extrap{Energy-efficient Ethernet}
\fxwarning{Write something about \acf{EEE}.}

\extrap{Power-over-Ethernet}
\fxwarning{Write something about \acs{POE}.}

\extrap{Ethernet \acs{OAM}}
...
Thus one of the frames needs to be stored in a temporary memory buffer while the other is being sent over the wire.


\extrap{store-and-forward}
   \index{forwarding!store-and-forward}
   \index{forwarding!cut-through}
When a switch receives the entire Ethernet frame into its buffer, calculates and verifies the \acs{FCS}, and only then forwards the frame further towards its destination, it is called \emph{store-and-forward} switching.
...
\paragraph{time to live?}
   \index{network loop}
   \iacs{TTL}
Ethernet, unlike the \acl{IP}, has no \acl{TTL} field so when a loop gets created in the network, the frames will keep traversing the network, bringing the entire network down.
...
\item
   ...
   \item Forward the frame out all interfaces except interface~1.
