...
\mode<article>{
A \ac{MAC} address is a unique identifier assigned to a \ac{NIC} for use as an address in communications within a network segment.
...
Something the address is displayed as three groups of four hexadecimal digits, separated by dots, e.g. 0200.33aa.bbcc.%
   \footnote{Note that hexadecimal numbers are not case-sensitive so 4F is equal to 4f.}
}
