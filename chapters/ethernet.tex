...
\mode<article>{
Ethernet has no time-to-live field so when a loop gets created in the network, the frames will keep traversing the network, bringing the entire network down.
}
...
Layer~3 switches can route packets in hardware based on IP address as if they were routers.
